...
**3. 자모음 인식**
* 원래는 자모음까지 데이터셋을 넣으려고 했습니다. 수화에서 자모음은 보통 이름을 말하거나 단어가 기억나지 않을 때 사용한다고 해서 필수적이라고 생각했습니다. 하여 RandomForest 모델을 이용하여 ㄱ부터 ㅣ까지 자모음을 추가했었습니다. 하지만 1과 같은 이유로 자모음과 일반 단어들이 혼동되어 인식되는 경우가 다분하여 기입하지 않았습니다.
